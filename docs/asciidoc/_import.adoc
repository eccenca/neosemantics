...
The URIs identifying the elements in the RDF data (resources, properties, etc) have their namespace part shortened to make them more human readable and easier to query with Cypher.

In our example, `http://neo4j.org/vocab/sw#name` has been shortened to `neo4voc\_\_name` (notice the double underscore separator used between the prefix and teh local name in the URI).
Similarly, `http://www.w3.org/1999/02/22-rdf-syntax-ns#type` would be shortened to `rdf\_\_type` and so on...

Prefixes explicitly defined in the RDF document are added to the graph's namespace mapping (provided that no mapping for the URI already exists).
Prefixes for non-shortened URIs are assigned sequentially (`ns0`, `ns1`, etc) as they appear in the imported RDF.
...
When applying url shortening on RDF ingestion (either explicitly or implicitly), we have the option of letting neosemantics automatically assign prefixes to namespaces as they appear in the imported RDF or inherit the ones defined in the RDF document (provided that the mapped namespaces are not already part of the graph's namespace mapping).
But before doing that, a few popular ones will be set with familiar prefixes.
These include `"http://www.w3.org/1999/02/22-rdf-syntax-ns#"` prefixed as `rdf` and `"http://www.w3.org/2004/02/skos/core#"` prefixed as `skos`.
